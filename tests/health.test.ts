...
async function withProcess(app: string, env: NodeJS.ProcessEnv, run:(base:string)=>Promise<void>) {
 ...
 const child=spawn(process.execPath,['--experimental-strip-types',`apps/${app}/src/index.ts`],{env:{...process.env,...env,API_PORT:String(port),WORKER_PORT:String(port)},stdio:'ignore'});
 const base=`http://127.0.0.1:${port}`;
 try {
  let started=false;
  for(let i=0;i<100;i++) {
   if(child.exitCode!==null) throw new Error(`${app} exited before health`);
   try { if((await fetch(`${base}/health`)).ok) {started=true;break;} } catch { /* startup */ }
...
test('API health, unavailable readiness, protected session and absent admin route',async()=>{
 ...
 await withProcess('api',{SUPABASE_URL:`http://127.0.0.1:${offline}`,SUPABASE_JWT_ISSUER:`http://127.0.0.1:${offline}/auth/v1`},async base=>{
